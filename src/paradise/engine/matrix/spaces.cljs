(ns paradise.engine.matrix.spaces
  (:require
   ...
   [paradise.engine.matrix.rooms :refer [process-parent-queue!]]
...
(bind/register! :matrix :paginate-space
  (fn [{:keys [space-id]}]
...
(bind/register! :matrix :space-add-child
...
(bind/register! :matrix :space-remove-child
  (fn [{:keys [space-id child-id]}]
    (go
      (if-let [svc @!space-service]
        (try
