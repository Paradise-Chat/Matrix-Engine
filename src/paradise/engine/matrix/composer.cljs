(ns paradise.engine.matrix.composer
(:require [cljs.core.async :refer [go]]
            ...
            [paradise.engine.matrix.timeline :refer [!active-timelines]]
            ["ffi-bindings" :as sdk :refer [MessageFormat UploadSource UploadParameters]]
            [cljs.core.async.interop :refer-macros [<p!]]))

(bind/register! :matrix :upload-media
  (fn [{:keys [mime buffer]}]
    (go
      (try
...
(bind/register! :matrix :send-message
...
(bind/register! :matrix :send-attachments
...
(bind/register! :matrix :send-raw-sticker
...
(bind/register! :matrix :persist-draft
...
(bind/register! :matrix :load-draft
  (fn [{:keys [room-id]}]
    (go
      (try
        (if-let [client @state/!client]
...
(bind/register! :matrix :fetch-emotes
