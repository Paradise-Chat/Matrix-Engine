(ns paradise.engine.matrix.rooms
  (:require
   ...
   [paradise.engine.matrix.diffs :refer [apply-matrix-diffs]]
   [net :as net]
...
(bind/register! :matrix :get-room-preview
                 (fn [{:keys [room-id]}]
                   (go
                     (try
...
(bind/register! :matrix :set-room-filter
  (fn [{:keys [filter-id]}]
...
(bind/register! :matrix :paginate-room-list
  (fn [_]
    (if-let [ctrl @!ui-controller]
...
(bind/register! :matrix :fetch-space-hierarchy
                 (fn [{:keys [space-id]}]
                   (go
                     (try
...
(bind/register! :matrix :fetch-room-membership
                 (fn [{:keys [room-id]}]
                   (go
...
(bind/register! :matrix :fetch-room-security
                 (fn [{:keys [room-id]}]
                   (go
                     (if-let [client @state/!client]
                       (try
...
(bind/register! :matrix :create-room
...
(bind/register! :matrix :join-room
                (fn [{:keys [room-id]}]
                   (go
                     (if-let [client @state/!client]
                       (try
...
(bind/register! :matrix :leave-room
  (fn [{:keys [room-id]}]
    (go
      (if-let [client @state/!client]
        (try
...
(bind/register! :matrix :invite-user
...
(bind/register! :matrix :knock-room
                 (fn [{:keys [room-id]}]
                   (go
                     (if-let [client @state/!client]
                       (try
...
(bind/register! :matrix :copy-room-link
                 (fn [{:keys [room-id]}]
                   (go
                     (if-let [client @state/!client]
                       (try
