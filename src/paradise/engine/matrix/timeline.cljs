(ns paradise.engine.matrix.timeline
  (:require
...
(bind/register! :matrix :boot-timeline
...
(bind/register! :matrix :cleanup-timeline
  (fn [{:keys [room-id]}]
...
(bind/register! :matrix :paginate-timeline
...
(bind/register! :matrix :toggle-reaction
...
(bind/register! :matrix :toggle-pin
...
(bind/register! :matrix :mark-read
...
(bind/register! :matrix :send-receipt
  (fn [{:keys [room-id event-id]}]
    (go
      (if-let [tl (get-in @!active-timelines [room-id :live :timeline])]
        (try
...
(bind/register! :matrix :redact-event
  (fn [{:keys [room-id msg-id]}]
    (go
      (if-let [tl (get-in @!active-timelines [room-id :live :timeline])]
        (try
...
(bind/register! :matrix :get-media
...
(bind/register! :matrix :get-pinned-events
  (fn [{:keys [room-id]}]
    (go
      (if-let [room (.getRoom @state/!client room-id)]
        (try
...
(bind/register! :matrix :search-room
...
(bind/register! :matrix :get-event
                 (fn [{:keys [room-id event-id]}]
                   (go
                     (try
                       (if-let [room (.getRoom @state/!client room-id)]
